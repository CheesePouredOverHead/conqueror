...
class ItemEncoder(nn.Module):
    ...
    def __init__(self, item_obs_size=39, item_num=180, item_encoder_type='TF', hidden_size=64, activation=nn.ReLU()):
...
class SheepModel(nn.Module):
    ...
    def __init__(self, item_obs_size=39, item_num=180, item_encoder_type='TF', stack_obs_size=30, global_obs_size=19, hidden_size=64, activation=nn.ReLU(), ttorch_return=False):
    ...
    def compute_actor(self, x):
        ...
        stack_embedding = self.stack_encoder(x['stack_obs'])
        global_embedding = self.global_encoder(x['global_obs'])

        key = item_embedding
        query = stack_embedding + global_embedding
    ...
    def compute_critic(self, x):
        ...
        embedding = item_embedding.mean(1) + stack_embedding + global_embedding
