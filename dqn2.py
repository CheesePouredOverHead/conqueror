...
register(
    id="SheepEnv-v0",
    entry_point="sheepenv2:SheepEnv",
...
class Agent:
    ...
    def choose_action(self, state):
        ...
        # print(state['pile'])
        legal_actions = [i for i in range(self.action_size) if state['pile'][i][4]==1]

        if len(legal_actions) == 0:
            return None  
        if np.random.rand() <= self.epsilon:
            return random.choice(legal_actions)
        state_vector = np.concatenate([state["pile"].flatten(),state['stack']])
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            state = np.concatenate([state["pile"].flatten(),state['stack']])
            state = torch.FloatTensor(state)
            next_state = np.concatenate([next_state["pile"].flatten(),next_state['stack']])
...
if __name__=='__main__':
    ...
    state_size = np.prod(env.observation_space.spaces["stack"].shape) + np.prod(env.observation_space.spaces["pile"].shape) 
    # print(state_size)
    action_size = env.action_space.n
    # print(state_size, action_size)
    
    agent = Agent(state_size, action_size)
    agent.model.load_state_dict(torch.load('model_parameters2.pth'))
    ...
    for e in range(200):
        ...
        with tqdm(total=int(EPISODES / 10), desc='Iteration %d' % e) as pbar:
            ...
            for time in range(int(EPISODES / 10)):
                ...
                # print(action)
                next_state, reward, done,f, _ = env.step(action)
        ...
        torch.save(agent.model.state_dict(), 'model_parameters2.pth')
