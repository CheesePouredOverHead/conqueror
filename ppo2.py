...
from sheepenv5 import SheepEnv
from model2 import SheepModel
import torch

sheep_ppo_config = dict(
    exp_name='sheep_ppo_env5',
    env=dict(
        env_id='Sheep-v0',
        floor=3,
        collector_env_num=8,
...
def main(input_cfg, seed, max_env_step=int(1e7), max_train_iter=int(1e7)):
    """
    The main function that runs the sheep elimination program.

    Args:
        input_cfg (tuple): A tuple containing the configuration parameters.
        seed (int): The seed value for random number generation.
        max_env_step (int, optional): The maximum number of environment steps. Defaults to 10000000.
        max_train_iter (int, optional): The maximum number of training iterations. Defaults to 10000000.
    """
    # 配置各种
