...
class SheepEnv(gym.Env):
    ...
    def __init__(self,floor):
        ...
        self.pile.setting=12*floor
        self.pile.floor=floor
        self.pile.cardnumber=[12 for x in range(self.pile.floor)]
    ...
    def score2(self,step,card,canmove,cansee,dicc,rest):
        ...
        if rest==0:
            ...
            canmove.remove(card)
            cansee.remove(card)
            dicc[int(card.get_no())].append(self.stack.inside+step)
    ...
    def step(self, action):
        ...
        if self.movable_cards[action]==0 or self.pile.lst[action].up!=[]:
            ...
            reward=self.score2(1,self.pile.lst[action],copy.copy(canmove),copy.copy(cansee),copy.deepcopy(self.stack.dic),self.stack.capacity-self.stack.inside)
