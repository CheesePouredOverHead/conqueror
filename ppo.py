...
from sheepenv4 import SheepEnv
from sheep_model import SheepModel
import torch

sheep_ppo_config = dict(
    exp_name='sheep_ppo_12_2_return10',
    env=dict(
        env_id='Sheep-v0',
        floor=2,
        collector_env_num=8,
...
def main(input_cfg, seed, max_env_step=int(1e7), max_train_iter=int(1e7)):
    ...
    model = SheepModel(
        ...
        'TF',
        obs_space['stack_obs'].shape[0],
        obs_space['global_obs'].shape[0]
    )
    ckpt = torch.load('./sheep_ppo_12_2_return10/ckpt\ckpt_best.pth.tar', map_location='cpu')['model']
